...
public class GenerateSortedArrayFromTwoArraysAlternate {

	/*
	 * Function to generates and prints all sorted arrays from alternate
	 * elements of 'A[i..m-1]' and 'B[j..n-1]' If 'flag' is true, then current
	 * element is to be included from A otherwise from B. 'len' is the index in
	 * output array C[]. We print output array each time before including a
	 * character from A only if length of output array is greater than 0. We try
	...
	void generateUtil(int A[], int B[], int C[], int i, int j, int m, int n, int len, boolean flag) {
		if (flag) // Include valid element from A
		{
			...
			for (int k = i; k < m; k++) {
				if (len == 0) {
					/*
					 * this block works for the very first call to include the
	...
	public static void main(String[] args) {
		GenerateSortedArrayFromTwoArraysAlternate generate = new GenerateSortedArrayFromTwoArraysAlternate();
		int A[] = { 10, 15, 25 };
		int B[] = { 5, 20, 30 };
		int n = A.length;
		int m = B.length;
