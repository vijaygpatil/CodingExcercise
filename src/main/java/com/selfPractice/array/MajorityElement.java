...
class MajorityElement {
	...
	/*
	 * If x is present in arr[low...high] then returns the index of first
	...
	static int _binarySearch(int arr[], int low, int high, int x) {
		if (high >= low) {
			...
			/*
			 * Check if arr[mid] is the first occurrence of x. arr[mid] is first
			 * occurrence if x is one of the following is true: (i) mid == 0 and
	...
	/*
	 * This function returns true if the x is present more than n/2 times in
	 * arr[] of size n
	 */
